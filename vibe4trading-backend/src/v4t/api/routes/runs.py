...
from v4t.api.schemas import PricePoint, RunCreateRequest, RunLeaderboardEntry, RunOut, TimelinePoint
...
from v4t.contracts.payloads import MarketPricePayloadV1
...
def create_run(
    ...
    if req.model_token_pairs:
        ...
        for pair in req.model_token_pairs:
            assert_model_predefined(db, pair.model_key)
    else:
    ...
    if req.model_token_pairs and len(req.model_token_pairs) > 1:
        ...
        child_runs = []
        for pair in req.model_token_pairs:
            child_cfg = RunConfigSnapshotV1(
                mode=RunMode.replay,
                market_id=req.market_id,
                model=ModelConfigV1(key=pair.model_key),
                datasets=DatasetRefsV1(
